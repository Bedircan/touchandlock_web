                    ...
                    <div class="thumbnail">
                        <div class="view overlay hm-zoom hm-black-light">
                            <a href="/details/{{$property->id}}">
                                <img src="{{isset($property->specs['img_1']) ? $property->specs['img_1'] : '../img/default-home.png'}}" class="img-fluid " style="width: 245px; height: 245px;" alt="">
                        ...
                        <div class="card-header">
                            <h2 class="price-property">₺{{$property->price}}</h2>

                            <h2 class="title-property" id="title-{{$counter}}"></h2>
                            <script type="text/javascript">
                                $('#title-{{$counter}}').text(getSubTitle("{!! $property->title !!}"))
                            </script>
                            <div>
                                <small class="text-two-line" id="desc-{{$counter}}">
                                </small>
                                <script type="text/javascript">
                                    $('#desc-{{$counter}}').text(getSubDesc("{!! $property->description !!}"));
                                </script>
                            </div>
                            <div class="text-center">
                                <a href="/reserve/{{$property->id}}" class="btn btn-primary center-block" role="button">Reserve</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <nav>
            <ul class="pagination">
                <li class="page-item active" id="page-1">
                    ...
                    <a class="page-link" href="{{route($routeDirection)}}?page=1{{isset($keywords) ? '&keywords='.$keywords : ''}}">1 <span class="sr-only">(current)</span></a>
                </li>
                @php($counter=($numberOfProperty/12)+1)
                @for ($i = 2; $i < $counter; $i++)
                    <li class="page-item" id="page-{{$i}}"><a class="page-link" href="{{route($routeDirection)}}?page={{$i}}{{isset($keywords) ? '&keywords='.$keywords : ''}}">{{$i}}</a></li>
                @endfor
            </ul>
        </nav>
