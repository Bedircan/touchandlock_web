...
@section('content')
    ...
    <div class="container">
        <div class="table">
            ...
            <div class="row">
                <div class="cell">
                    @if($properties[$i]['specs']['img_1'] == null)
                        <img style="width: 50px; height: 50px;" src="../img/default-home.png">
                    @else
                        <img style="width: 50px; height: 50px;" src="{{$properties[$i]['specs']['img_1']}}">
                    @endif
                    <a href="{{'https://www.team-8.tk/details/'.$properties[$i]->id}}">{{$properties[$i]->title}}</a>
                </div>
                <div class="cell">₺ {{$properties[$i]->price}}</div>
                <div class="cell">{{$properties[$i]->id}}</div>
                <div class="cell">{{$properties[$i]->type}}</div>
                <div class="cell">{{$properties[$i]->from_date}}</div>
                <div class="cell">{{$properties[$i]->to_date}}</div>
                <div class="cell">{{$properties[$i]->active == 1 ? 'ACTIVE' : 'PASSIVE'}}</div>
                <div class="cell"><a href="{{url('/account/properties/delete')}}{{'?prop_id='.$properties[$i]->id}}">Delete</a></div>
            </div>
            @endfor
        </div>
